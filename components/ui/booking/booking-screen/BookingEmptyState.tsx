...
export default function BookingEmptyState({ type }: BookingEmptyStateProps) {
  ...
  if (type === "upcoming") {
    return (
      <View className="flex-1 items-center justify-center py-12">
        ...
        <AppText className="text-text-secondary text-center mt-4 " variant="medium">No upcoming stays</AppText>
        <TouchableOpacity
          onPress={() => push("/(tabs)/home")}
          className="mt-4 bg-primary py-2 px-6 rounded-xl"
        >
          <AppText className="text-white" variant="bold">Explore Hotels</AppText>
        </TouchableOpacity>
      </View>
    );
  }
  ...
  return (
    <View className="flex-1 items-center justify-center py-12">
      ...
      <AppText className="text-text-secondary text-center mt-4" variant="medium">No booking history</AppText>
    </View>
  );
}